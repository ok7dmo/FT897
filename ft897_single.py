...
def parse_image(image: bytes) -> List[MemoryChannel]:
    ...
    for idx in range(TOTAL_CHANNELS):
        ...
        b0, b1, b2, b3, b4, b5, b6, b7 = chunk[:8]
        ...
        rx_freq = bcd_to_int(chunk[10:14]) * 10
        off_val = bcd_to_int(chunk[14:18]) * 10
        ...
        name = (
            chunk[18:26].replace(b"\xff", b" ").decode("ascii", "ignore").strip()
        )
...
def build_image(channels: Iterable[MemoryChannel]) -> bytes:
    ...
    for ch in channels:
        ...
        if ch.ctcss is not None and ch.dcs is None:
            block[4] = (block[4] & 0xFC) | 0x01
            block[6] = (block[6] & 0xC0) | int(ch.ctcss * 10) & 0x3F
        elif ch.dcs is not None and ch.ctcss is None:
            block[4] = (block[4] & 0xFC) | 0x02
            block[7] = (block[7] & 0x80) | (ch.dcs & 0x7F)
        elif ch.ctcss is not None and ch.dcs is not None:
            block[4] = (block[4] & 0xFC) | 0x03
            block[6] = (block[6] & 0xC0) | int(ch.ctcss * 10) & 0x3F
            block[7] = (block[7] & 0x80) | (ch.dcs & 0x7F)
        else:
            block[4] = (block[4] & 0xFC)

        block[10:14] = int_to_bcd(rx_units, 8)
        block[14:18] = int_to_bcd(off_units, 8)

        name = (ch.name or "").ljust(8)[:8]
        block[18:26] = name.encode("ascii", "ignore")
