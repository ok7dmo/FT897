...
# Memory layout of the FT-897 image.  The bulk of the file is a table
# of 200 memory entries starting at offset 0x431.  Each entry is 26
# bytes long, mirroring the structure used by CHIRP for the FT‑817/857
# series.  The overall image is 6509 bytes.
MEMORY_OFFSET = 0x431
CHANNEL_SIZE = 26
TOTAL_CHANNELS = 200
IMAGE_SIZE = 6509
...
def parse_image(image: bytes) -> List[MemoryChannel]:
    ...
    Only a subset of the fields is interpreted: RX frequency, TX
    frequency (derived from the offset fields), mode and channel name.
    """
    ...
    for idx in range(TOTAL_CHANNELS):
        ...
        rx_freq = int.from_bytes(chunk[10:14], "big") * 10
        off_val = int.from_bytes(chunk[14:18], "big") * 10
...
def build_image(channels: Iterable[MemoryChannel]) -> bytes:
    ...
    for ch in channels:
        ...
        if ch.tx_freq_hz == ch.rx_freq_hz:
            duplex_code = 0
            off_units = 0
        elif ch.tx_freq_hz > ch.rx_freq_hz and (ch.tx_freq_hz - ch.rx_freq_hz) < 10_000_000:
            duplex_code = 2
            off_units = int((ch.tx_freq_hz - ch.rx_freq_hz) // 10)
        elif ch.tx_freq_hz < ch.rx_freq_hz and (ch.rx_freq_hz - ch.tx_freq_hz) < 10_000_000:
            duplex_code = 1
            off_units = int((ch.rx_freq_hz - ch.tx_freq_hz) // 10)
        else:
            duplex_code = 3
        ...
        b0 = mode_code & 0x07
        b1 = (duplex_code & 0x03) << 6

        image[start] = b0
        image[start + 1] = b1 | (1 if duplex_code else 0) << 5
        image[start + 10:start + 14] = rx_units.to_bytes(4, "big")
        image[start + 14:start + 18] = off_units.to_bytes(4, "big")
        name = (ch.name or "")[:8].ljust(8)
        image[start + 18:start + 26] = name.encode("ascii", "ignore")
...
def export_csv(channels: Iterable[MemoryChannel], fp) -> None:
    ...
    writer.writerow(
        ["index", "rx_freq_hz", "tx_freq_hz", "mode", "name"]
    )
    for ch in channels:
...
class RadioControlApp(QMainWindow):
    ...
    def init_menu(self):
        ...
        load_act = QAction("Načíst z rádia…", self)
        ...
        import_act = QAction("Importovat…", self)
    ...
    def load_from_radio(self):
        ...
        clone = FT897Clone(self.cat.port)
        image = clone.recv_image()
        self.memory_model.set_from_image(image)
    ...
    def save_to_radio(self):
        ...
        QMessageBox.information(self, "Instrukce", FT897Clone.enter_clone_mode_instructions())
        clone = FT897Clone(self.cat.port)
        clone.send_image(self.memory_model.build_image())
    ...
    def import_memories(self):
        ...
        if path.endswith(".img"):
            with open(path, "rb") as f:
                self.memory_model.set_from_image(f.read())
