...
class FT897CAT:
    ...
    def __init__(self) -> None:
        """Initialize default CAT settings and state."""
    ...
    def _find_rigctl_path(self) -> str:
        """Return path to rigctl executable.

        Searches PATH and common Windows install locations.
        Raises:
            FileNotFoundError: if rigctl.exe cannot be located.
        """
    ...
    def _readline_with_timeout(self, timeout: float) -> Optional[str]:
        """Return a line from rigctl stdout or None on timeout."""
    ...
    def _start_rigctl(self) -> None:
        """Launch a persistent rigctl process if not already running."""
        if self.proc:
            return
        ...
        if self.proc.stdout:
            ...
            while time.time() - start < 1:
                line = self._readline_with_timeout(0.1)
                if not line:
                    break

    def _send_command(self, command: str) -> Optional[str]:
        """Send a command to rigctl and return its response line."""
        try:
            self._start_rigctl()
            ...
            with self._lock:
                ...
                response = self._readline_with_timeout(1)
                if response is not None:
                    # Drain any remaining output such as prompts
                    while self._readline_with_timeout(0.05):
                        pass
                    self.last_error = None
                    return response.strip()
                self.last_error = "Timeout waiting for rigctl"
        except FileNotFoundError:
            raise
        except Exception as exc:
            self.last_error = str(exc)
            logger.warning("rigctl command failed: %s", exc)
        return None

    def connect(self, port: str, baudrate: int = 9600) -> bool:
        """Connect to the radio over the given serial port.

        Raises:
            FileNotFoundError: if rigctl.exe cannot be found.
        """
        self.port = port
    ...
    def disconnect(self) -> None:
        """Terminate rigctl process and clear connection state."""
        if self.proc:
            try:
                self.proc.terminate()
    ...
    def get_frequency(self) -> Optional[int]:
        ...
        if res:
            try:
                ...
                logger.warning("Neplatná frekvence: %s", res)
        return None
    ...
    def set_mode(self, mode: str) -> bool:
        """Set operating mode (e.g. LSB, USB)."""
    ...
    def ptt_on(self) -> bool:
        """Activate the transmitter (PTT on)."""
        if not self.is_connected:
            return False
    ...
    def ptt_off(self) -> bool:
        """Deactivate the transmitter (PTT off)."""
        if not self.is_connected:
            return False
    ...
    def get_smeter(self) -> Optional[int]:
        ...
        if res:
            try:
                ...
                logger.warning("Neplatná hodnota S-měru: %s", res)
        return None

    def read_tx_status(self) -> Optional[int]:
        """Return combined TX status byte or None on error."""
        if not self.is_connected:
            return None
        ...
        if power_res:
            try:
                ...
                logger.warning("Neplatná hodnota výkonu: %s", power_res)
        return max(0, min(level, 15))

    def get_power_level(self) -> Optional[int]:
        """Return power level from last TX status or None."""
    ...
    def get_swr(self, tx_status: Optional[int] = None) -> Optional[int]:
        """Return SWR flag (1=high, 0=normal) or None on error."""
        if not self.is_connected:
            return None
        ...
        if res:
            try:
                ...
                logger.warning("Neplatné SWR: %s", res)
        return None
    ...
    def get_repeater_offset_mode(self) -> Optional[str]:
        """Return current repeater shift mode."""
        if not self.is_connected:
            return None
    ...
    def apply_repeater_settings(
        ...
        """Apply repeater offset and optional CTCSS tone."""
        if not self.is_connected:
            return
        ...
        if tone_hz is not None:
            mode = "TONE" if tx_only else "TSQL"
            self._send_command(f"set_ctcss_mode {mode}")
    ...
    def set_ctcss_dcs_mode(self, mode: str) -> bool:
        """Set CTCSS or DCS operating mode."""
        if not self.is_connected:
            return False
        mapping = {
            "dcs": "DCS",
    ...
    def set_ctcss_tone(self, tone_hz: float) -> bool:
        """Set CTCSS tone frequency in Hz."""
        if not self.is_connected:
            return False
        return self._send_command(
            f"set_ctcss_tone {int(round(tone_hz))}"
        ) is not None

    def set_dcs_code(self, tx_code: int, rx_code: int) -> bool:
        """Set DCS transmit and receive codes."""
        if not self.is_connected:
            return False
