...
class FT897CAT:
    ...
    def _run(self, *args: str) -> Optional[subprocess.CompletedProcess[str]]:
        if not self.rigctl_path:
            try:
                ...
                return None
        # 1023 = FT-897 model number in Hamlib's rig database
        cmd = [self.rigctl_path, '-m', '1023']
        if self.port:
            cmd += ['-r', self.port]
        if self.baudrate:
        ...
        try:
            ...
            msg = getattr(e, 'stderr', str(e)) or str(e)
            self.last_error = msg.strip()
    ...
    def connect(self, port: str, baudrate: int = 9600) -> bool:
        ...
        self.port = port
        self.baudrate = 9600
        if self._run('get_freq'):
            self.is_connected = True
            logging.getLogger(__name__).info(
                'Connected to %s at %d bps', port, self.baudrate
            )
            return True
        self.is_connected = False
        if not self.last_error:
            self.last_error = 'Nepodařilo se připojit'
        return False
    ...
    def set_frequency(self, freq_hz: int) -> bool:
        ...
        return self._run('set_freq', f"{mhz:.6f}") is not None
...
class RadioControlApp(QMainWindow):
    ...
    def update_status(self, freq_hz, sm_level, power_level, swr_flag):
        ...
        if freq_hz >= 0:
            ...
            mhz = freq_hz // 1_000_000
            fraction = (freq_hz % 1_000_000) // 10
            formatted = f"{mhz:03d}.{fraction:05d}"
