    ...
    def connect(self, port: str, baudrate: int = 9600) -> bool:
        ...
        if self.use_hamlib and self._rig is not None:
            try:  # pragma: no cover - depends on Hamlib
                ...
                self._rig.open()
                # Verify the radio responds before declaring success
                self._rig.get_freq()
        ...
        try:
            ...
            # Ensure rigctl actually talks to the radio
            if self._run('get_freq') is None:
                raise RuntimeError(self.last_error or 'rigctl did not respond')
            self.is_connected = True
