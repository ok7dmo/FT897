...
class FT897CAT:
    ...
    def apply_repeater_settings(self,
                                offset_hz: int,
                                tone_hz: Optional[float] = None,
                                tx_only: bool = False) -> None:
        """
        • Zapne split režim (`split_on()`)
        • Nastaví směr offsetu:
          - `offset_hz > 0`: "plus"
          - `offset_hz < 0`: "minus"
          - `offset_hz == 0`: "simplex"
        • Nastaví offset frekvenci na `abs(offset_hz)`
        • Pokud `tone_hz` není `None`:
          - přepne do CTCSS enkód (TX-only pokud `tx_only=True`, jinak běžný CTCSS)
          - nastaví sub-ton `tone_hz`
        • Jinak:
          - přepne do režimu "off" (`set_ctcss_dcs_mode("off")`)
        """
        self.split_on()
        # 1) offset mode
        if offset_hz > 0:
            ...
            mode = "simplex"
        self.set_repeater_offset_mode(mode)
        # 2) offset frequency
        self.set_repeater_offset_frequency(abs(offset_hz))
        # 3) CTCSS
        if tone_hz is not None:
            self.set_ctcss_dcs_mode("ctcss_enc" if tx_only else "ctcss")
            self.set_ctcss_tone(tone_hz)
        else:
