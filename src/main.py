...
import serial
# Hamlib bindings use different module names on various platforms.  The
# Windows wheels install `hamlib`, while many Linux distributions provide
# `Hamlib`.  Try the lowercase import first for Windows compatibility and
# fall back to the capitalised variant.
try:
    import hamlib  # type: ignore
except ModuleNotFoundError:  # pragma: no cover - optional dependency
    try:
        import Hamlib as hamlib  # type: ignore
    except ModuleNotFoundError as exc:  # pragma: no cover
        raise ImportError(
            "Hamlib bindings are required. Install the python-hamlib package."
        ) from exc
...
class SerialCommunicationError(CATError):
...
class FT897CAT:
    ...
    def send_command(self, cmd: bytes, resp_len: int = 0):
        ...
        Up to three attempts are made to guard against transient
        communication glitches.  This helps the S-metr and výkon
        readings remain "to be" rather than "not to be".
        """
        for attempt in range(3):
            with self._lock:
                try:
                    ...
                    logger.warning(
                        "Serial command failed (attempt %d/3): %s", attempt + 1, e
                    )
                    time.sleep(0.1)
        logger.error("Serial command failed after retries")
        raise SerialCommunicationError("Serial command failed after retries")
    ...
    # Allow use in a with-statement for automatic cleanup
    def __enter__(self):
        return self
    ...
    def get_repeater_offset_mode(self):
        """Return current repeater shift direction if the radio reports it."""
        if not self.is_connected:
            return None
        resp = self.send_command(b"\x00\x00\x00\x00\x09", resp_len=5)
        if not resp or len(resp) != 5:
            return None
        mapping = {0x09: "minus", 0x49: "plus", 0x89: "simplex"}
        return mapping.get(resp[0])

    def apply_repeater_settings(
        self, offset_hz: int, tone_hz: Optional[float] = None, tx_only: bool = False
    ) -> None:
        """Configure repeater shift, offset and CTCSS using Hamlib only."""

        if hamlib is None:  # pragma: no cover - import guarded above
            raise RuntimeError("Hamlib bindings are required for repeater setup")

        rig = hamlib.Rig(1023)
        rig.set_conf("rig_pathname", self.port)
        rig.open()
        shift = '-' if offset_hz < 0 else '+' if offset_hz > 0 else '0'
        rig.set_rptr_shift(shift)
        rig.set_rptr_offs(abs(offset_hz))
        if tone_hz is not None:
            rig.set_ctcss_tone(int(round(tone_hz * 10)))
            rig.set_ctcss_mode(
                hamlib.RIG_CTCSS_TX if tx_only else hamlib.RIG_CTCSS_TSQL
            )
        else:
            rig.set_ctcss_mode(hamlib.RIG_CTCSS_OFF)
        rig.close()
    ...
    def set_ctcss_dcs_mode(self, mode: str):
        ...
        mapping = {
            ...
            "ctcss": 0x2A,
            "ctcss_dec": 0x3A,
            # 0x4A selects "TONE" (CTCSS encode only – TX sub-tone without RX decode)
            "ctcss_enc": 0x4A,
            "off": 0x8A,
        }
        code = mapping.get(mode)
    ...
    def set_ctcss_tone(self, tone_hz: float) -> bool:
        """Nastaví CTCSS tón pro vysílání i příjem.

        Podle implementace v Hamlibu FT‑897 nepodporuje rozdílné
        sub‑tóny pro TX a RX, proto posíláme stejný kód pro obě směry.
        """
        if not self.is_connected:
            return False
        tone = int(round(tone_hz * 10))
        digits = f"{tone:04d}"
        data = bytearray()
        for i in range(0, 4, 2):
    ...
    def toggle_connection(self) -> None:
        if not self.cat.is_connected:
            ...
            if self.cat.connect(port):
                ...
                self.show_toast("Nelze se připojit.")
        else:
