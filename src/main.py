        ...
        if repeater:
            ...
            if offset_val is None:
                if 145_000_000 <= freq <= 146_000_000:
                    ...
                    offset_val = -7_600_000

            if offset_val:
                self.cat.set_repeater_offset_frequency(abs(int(offset_val)))
                self.cat.set_repeater_offset_mode("minus" if offset_val < 0 else "plus")
            else:
                self.cat.set_repeater_offset_frequency(0)
                self.cat.set_repeater_offset_mode("simplex")

            # reapply tuned frequency so the shift engages
            self.cat.set_frequency(freq)
