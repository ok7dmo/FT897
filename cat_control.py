...
class FT897CAT:
    ...
    def get_meter_block(self) -> Optional[bytes]:
        ...
        Some radios require the full ``FE FE 44 00 E7 FD`` sequence
        instead of the shortened five-byte command.  This method uses
        the long form for maximum compatibility.
        """
        cmd = b"\xFE\xFE\x44\x00\xE7\xFD"
        if not self._send(cmd):
            return None
        resp = self._read(7)
        if not resp or len(resp) != 7:
            return None
        # Response layout: FE FE 00 44 E7 <S> <ALC> <SWR> <DEV> <VOLT> FD
        return resp[5:-1]
