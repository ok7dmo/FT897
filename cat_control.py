...
class FT897CAT:
    ...
    def get_s_and_power(self) -> Tuple[Optional[int], Optional[int]]:
        ...
        The rig echoes the opcode followed by the value byte.  We read just
        two bytes per command to avoid unnecessary waiting.
        """
        if not self._send(b"\x00\x00\x00\x00\xE7"):
            return None, None
        resp = self._read(2)
        s_val = resp[1] if resp and len(resp) >= 2 else None

        if not self._send(b"\x00\x00\x00\x00\xF7"):
            return s_val, None
        resp2 = self._read(2)
        p_val = resp2[1] if resp2 and len(resp2) >= 2 else None
        return s_val, p_val
