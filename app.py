...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.memory_presets = {
            ...
            "70 cm": [
                ("Rink", 433_275_000, "FM", 88.5, 0),
            ],
            "Převaděče": [
                ("Gecko", 145_587_500, "FM", 77.0, -600_000),
            ],
        }
    ...
    def init_menu(self):
        ...
        for group, memories in self.memory_presets.items():
            ...
            for mem in memories:
                ...
                payload = [int(freq), mode]
                if ctcss is not None:
                    payload.append(ctcss)
                if offset is not None:
                    payload.append(offset)
                act.setData(tuple(payload))
                act.triggered.connect(self.tune_from_menu)
                sub.addAction(act)
    ...
    def tune_from_menu(self):
        ...
        if not data or not self.cat.is_connected:
            return
        if len(data) >= 2:
            freq, mode = data[0], data[1]
        else:
            return
        ctcss = data[2] if len(data) >= 3 else None
