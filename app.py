        ...
        try:
            while True:
                ...
                if chunk:
                    ...
                    if len(data) >= min_len and data[-2:] == b"\xff\xff":
                        break
                else:
...
class RadioControlApp(QMainWindow):
    ...
    def load_memories_from_radio(self):
        port = self.port_combo.currentText()
        if not port:
            QMessageBox.warning(self, "Chyba", "Není vybrán žádný port.")
            return
        ...
        QMessageBox.information(
            self,
            "Clone Mode",
            "Vypněte rádio, podržte [→] a [←] a zapněte do Clone Mode.\n"
            "Poté stiskněte tlačítko [C] na rádiu."
        )

        if not self.cat.connect(port):
            QMessageBox.warning(self, "Error", "Nelze otevřít port pro Clone mód.")
            if was_connected:
                self.cat.connect(port)
                self.status_thread.start()
            return

        progress = QProgressDialog("Čtení pamětí...", "Cancel", 0, 8192, self)
        progress.setWindowTitle("Clone")
        progress.setWindowModality(Qt.WindowModal)

        raw = self.cat.fetch_clone_dump(progress_cb=progress.setValue)
        progress.close()
        if not raw:
            QMessageBox.warning(self, "Error", "Nepodařilo se přečíst paměti.")
            self.cat.disconnect()
            if was_connected:
                self.cat.connect(port)
                self.status_thread.start()
