...
class FT897CAT:
    ...
    # --- Clone mode handling ---
    def fetch_clone_dump(self) -> bytes | None:
        ...
        try:
            # Clone mode typically uses the same 9600 bps speed as CAT
            self.serial_port = serial.Serial(
                port=self.port,
                baudrate=9600,
                bytesize=serial.EIGHTBITS,
            ...
            print(f"Permission denied to port {self.port}.")
            return None
        except Exception as e:
...
class RadioControlApp(QMainWindow):
    ...
    def load_memories_from_radio(self):
        if not self.cat.is_connected:
            QMessageBox.warning(self, "Error", "Radio not connected.")
            return
        port = self.port_combo.currentText()
        self.cat.disconnect()
        ...
        if not self.cat.connect(port):
            QMessageBox.warning(self, "Error", "Could not reconnect to radio in Clone Mode.")
            return
        time.sleep(1)
