...
from PyQt5.QtWidgets import (
    ...
    QInputDialog, QFileDialog, QTableWidget, QTableWidgetItem
)
...
class MemoryEntry:
    ...
    freq_hz: int
    mode: str
    ctcss_tx: float | None = None
    ctcss_rx: float | None = None
    dcs_tx: int | None = None
    dcs_rx: int | None = None
    repeater_shift_hz: int | None = None
...
class FT897CAT:
    ...
    # --- Clone mode handling ---
    def fetch_clone_dump(
        self,
        min_len: int = 4096,
        idle_ms: int = 400,
        max_ms: int = 15000,
    ) -> bytes | None:
        """Read EEPROM data sent by FT-897 in Clone Mode after [C] is pressed."""
        if not self.port:
            return None
        temp_opened = False
        if not self.serial_port or not self.serial_port.is_open:
            if not self.connect(self.port, 9600):
                return None
            temp_opened = True
        # Increase timeout for clone streaming
        self.serial_port.timeout = 0.05

        data = bytearray()
        start = last = time.monotonic()
        logger.info("Waiting for Clone Mode data (press [C] on radio)...")
        try:
            while True:
                ...
                if chunk:
                    ...
                    if data and (now - last) * 1000 >= idle_ms:
                        break
                    if not data and (now - start) * 1000 >= max_ms:
                        print("Timeout waiting for clone data")
                        break
                if len(data) >= min_len and (now - last) * 1000 >= idle_ms:
                    break
        except Exception as e:
            logger.error("Clone read error: %s", e)
            data = bytearray()
        finally:
            if temp_opened:
                try:
                    self.serial_port.close()
                    logger.info("Clone port closed")
                except Exception:
                    pass

        if len(data) < min_len:
            logger.warning("Clone dump too short: %d bytes", len(data))
            return None
        logger.info(
            "Read %d bytes from clone stream: %s ... %s",
            len(data),
            data[:8].hex(),
            data[-8:].hex(),
        )
    ...
    def parse_clone_dump(self, data: bytes) -> list[MemoryEntry]:
        """Parse raw EEPROM dump from Clone Mode into MemoryEntry objects."""
        ...
        if len(data) < MEM_START + MEM_COUNT * MEM_SIZE:
            print(f"Clone dump too short: {len(data)} bytes")
            return entries

        filled = data[FILLED_START:FILLED_START + 25]
        ...
        for i in range(MEM_COUNT):
            if not (filled[i // 8] >> (i % 8)) & 1:
                continue
            off = MEM_START + i * MEM_SIZE
            block = data[off:off + MEM_SIZE]
            freq = int.from_bytes(block[10:14], "big") * 10
            mode_code = block[0] & 0x07
            mode = mode_map.get(mode_code, "FM")
            label_bytes = block[18:26]
            raw = label_bytes.split(b"\xff", 1)[0].split(b"\x00", 1)[0]
            label = raw.decode("ascii", "replace").strip()
            if not label:
                label = f"MEM {i+1}"
            entries.append(MemoryEntry(index=i+1, label=label, freq_hz=freq, mode=mode))

        logger.info("Parsed %d memory channels", len(entries))
        return entries
...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.memory_cache_path = Path.home() / ".ft897_memories.json"
        self.radio_memories: list[MemoryEntry] = []
    ...
    def init_ui(self):
        ...
        # Tab with memory channels read from the radio
        self.mem_tab = QWidget()
        self.mem_layout = QVBoxLayout(self.mem_tab)
        self.memory_table = QTableWidget(0, 4)
        self.memory_table.setHorizontalHeaderLabels(["#", "Label", "Freq (MHz)", "Mode"])
        self.memory_table.cellDoubleClicked.connect(self.memory_cell_double_clicked)
        self.mem_layout.addWidget(self.memory_table)
        self.tabs.addTab(self.mem_tab, "Paměti")
    ...
    def init_menu(self):
        ...
        load_radio_action = QAction("Load memories from radio…", self)
        load_radio_action.triggered.connect(self.load_memories_from_radio)
        self.memory_menu.addAction(load_radio_action)

        load_file_action = QAction("Load memories from file…", self)
        load_file_action.triggered.connect(self.load_memories_from_file)
        self.memory_menu.addAction(load_file_action)
    ...
    def load_memory_cache(self):
        """Populate Radio memories menu from cached JSON if present."""
        if not hasattr(self, "memory_menu"):
            return
        if not self.memory_cache_path.exists():
            return
        try:
            with open(self.memory_cache_path, "r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as e:
            print(f"Could not load memory cache: {e}")
            return
        self.radio_memories = []
        ...
        for entry_dict in data:
            try:
                ...
                continue
            self.radio_memories.append(entry)
            row = self.memory_table.rowCount()
            self.memory_table.insertRow(row)
            self.memory_table.setItem(row, 0, QTableWidgetItem(str(entry.index)))
            self.memory_table.setItem(row, 1, QTableWidgetItem(entry.label))
            freq_item = QTableWidgetItem(f"{entry.freq_hz/1_000_000:.5f}")
            freq_item.setData(Qt.UserRole, entry.freq_hz)
            self.memory_table.setItem(row, 2, freq_item)
            self.memory_table.setItem(row, 3, QTableWidgetItem(entry.mode))
    ...
    def memory_cell_double_clicked(self, row, column):
        if row >= len(self.radio_memories):
            return
        entry = self.radio_memories[row]
        if not self.cat.is_connected:
            QMessageBox.warning(self, "Chyba", "Rádio níč ní připojeno.")
            return
        self.cat.set_frequency(entry.freq_hz)
        self.cat.set_mode(entry.mode)

    def load_memories_from_radio(self):
        port = self.port_combo.currentText()
        if not port:
            QMessageBox.warning(self, "Error", "No serial port selected.")
            return

        was_connected = self.cat.is_connected
        if was_connected:
            self.status_thread.stop()
            self.status_thread.wait()
            self.cat.disconnect()
        ...
        QMessageBox.information(
            self,
            "Clone Mode",
            "Turn OFF the radio and hold [→] and [←] while powering ON.\n"
            "Do not press C yet. When ready, press OK."
        )

        if not self.cat.connect(port):
            QMessageBox.warning(self, "Error", "Could not open port for clone mode.")
            return

        QMessageBox.information(self, "Clone Mode", "Now press [C] on the radio to start transfer.")

        raw = self.cat.fetch_clone_dump()
        if not raw:
            QMessageBox.warning(self, "Error", "Failed to read EEPROM.")
            if was_connected:
                self.cat.connect(port)
                self.status_thread.start()
            return
        QMessageBox.information(self, "Clone Mode", f"Přijato {len(raw)} bajtů")
        entries = self.cat.parse_clone_dump(raw)
        self.radio_memories = entries
        self.memory_table.setRowCount(0)
        ...
        try:
            ...
            print(f"Could not save memory cache: {e}")
        if was_connected:
            self.cat.connect(port)
            self.status_thread.start()
    ...
    def load_memories_from_file(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            "Open clone image",
            "",
            "Clone Images (*.img *.bin);;All Files (*)",
        )
        if not path:
            return
        try:
        ...
        entries = self.cat.parse_clone_dump(raw)
        if not entries:
            QMessageBox.warning(self, "Error", "No memories found in file.")
            return
        self.radio_memories = entries
        self.memory_table.setRowCount(0)
        for entry in entries:
