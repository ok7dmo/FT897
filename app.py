...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.memory_presets = {
            ...
            "Převaděče": [
                # OK0B repeater with fixed -600 kHz shift and TX-only CTCSS
                ("OK0B", 145_587_500, "FM", 77.0, -600_000, True),
            ],
        }
    ...
    def tune_from_menu(self):
        ...
        if repeater:
            ...
            if action.text().startswith("OK0B"):
                self.cat.set_ctcss_dcs_mode("ctcss_enc")
                self.cat.set_ctcss_tone(77.0, 0.0)
        else:
            if offset is not None:
                if offset == 0:
                    self.cat.set_repeater_offset_mode("simplex")
