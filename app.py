...
class FT897CAT:
    ...
    def fetch_clone_dump(
        ...
        try:
            ...
            print(f"Clone mode connection error: {e}")
            return None

        data = bytearray()
        start = last = time.monotonic()
        print("Waiting for Clone Mode data (press [C] on radio)...")
        try:
            while True:
                chunk = ser.read(256)
                now = time.monotonic()
        ...
        if len(data) < min_len:
            return None
        print(f"Read {len(data)} bytes from clone stream")
        return bytes(data)
    ...
    def parse_clone_dump(self, data: bytes) -> list[MemoryEntry]:
        ...
        for i in range(MEM_COUNT):
            ...
            label = label_bytes.split(b"\xff", 1)[0].decode("ascii", "ignore").strip()
            if not label:
                label = f"MEM {i+1}"
...
class RadioControlApp(QMainWindow):
    ...
    def load_memory_cache(self):
        ...
        try:
            ...
            return
        radio_menu = self.memory_menu.addMenu("Radio")
        for entry_dict in data:
            try:
                entry = MemoryEntry(**entry_dict)
            except Exception:
                continue
            act = QAction(f"{entry.label} ({entry.freq_hz/1_000_000:.3f} MHz)", self)
            act.setData((entry.freq_hz, entry.mode))
            act.triggered.connect(self.tune_from_menu)
            radio_menu.addAction(act)
    ...
    def load_memories_from_radio(self):
        ...
        QMessageBox.information(
            self,
            "Clone Mode",
            "Please turn OFF your FT-897 radio.\n"
            "Hold the [→] and [←] buttons, then turn it ON to enter Clone Mode.\n\n"
            "Once in Clone Mode, press the [C] (CENT) button on the radio.\n"
            "The app will detect the incoming memory image and proceed automatically."
        )
        self.cat.port = port
