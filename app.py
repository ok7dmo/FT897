...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.memory_presets = {
            ...
            "Převaděče": [
                # Gecko repeater with fixed -600 kHz shift and TX-only CTCSS
                ("Gecko", 145_587_500, "FM", 77.0, -600_000, True),
            ],
        }
    ...
    def tune_from_menu(self):
        ...
        if repeater:
            ...
            if offset_hz:
                current = self.cat.get_repeater_offset_mode()
                if current != "minus":
                    self.cat.set_repeater_offset_mode("minus")
                self.cat.set_repeater_offset_frequency(offset_hz)
            else:
                self.cat.set_repeater_offset_mode("simplex")
                self.cat.set_repeater_offset_frequency(0)

            if action.text().startswith("Gecko"):
