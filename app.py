...
class FT897CAT:
    ...
    def parse_clone_dump(self, data: bytes) -> list[MemoryEntry]:
        ...
        if not data:
            return entries
        # Simple parser using FT-817/897 memory layout. Each memory
        # channel is 16 bytes. Frequency is stored in BCD starting at
        # offset 0. Mode stored at byte 4.
        for i in range(0, len(data), 16):
            ...
            mode = {
                ...
                0x08: "FM",
                0x06: "DIG",
            }.get(mode_byte & 0x0F, "FM")
            entries.append(MemoryEntry(index=i//16, label=f"MEM {i//16}", freq_hz=freq_hz, mode=mode))
        return entries
