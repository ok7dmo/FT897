...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.memory_presets = {
            ...
            "70 cm": [
                # Rink simplex channel with TX CTCSS 88.5 Hz
                ("Rink", 433_275_000, "FM", 88.5, 0, True),
            ],
            "Převaděče": [
                # Additional repeater entries can omit offset; it will be set
                # automatically based on frequency when selected.
                ("Gecko", 145_587_500, "FM", 77.0),
            ],
        }
    ...
    def tune_from_menu(self):
        action = self.sender()
        data = action.data()
        if not isinstance(data, dict) or not self.cat.is_connected:
            return
        freq = data.get("freq")
        mode = data.get("mode")
