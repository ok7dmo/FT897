    ...
    def run(self):
        ...
        while self.running:
            if self.cat.is_connected:
                ...
                power = self.cat.get_tx_meter() if self.cat.ptt_active else None
...
class RadioControlApp(QMainWindow):
    ...
    def update_power_label(self, band_label, meter_value):
        ...
        When transmitting (PTT active), use ``meter_value`` to estimate
        output power. Otherwise display the nominal maximum power for the
        detected band so the label shows a meaningful value even in receive
        mode.
        """
        ...
        if self.cat.ptt_active and meter_value >= 0 and max_power > 0:
