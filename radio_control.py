...
class RadioControlApp(QMainWindow):
    ...
    def update_status(self, freq_hz, sm_level, mode, power_meter):
        ...
        # Query meters directly so readings are up to date
        raw_s = self.cat.get_smeter()
        sm_str = format_smeter(raw_s) if raw_s is not None else "---"
        self.smeter_label.setText(f"S-metr: {sm_str}")

        raw_p = self.cat.get_tx_meter()
        pw_str = format_power(raw_p if raw_p is not None else 0, band_label)
