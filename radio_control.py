...
class FT897CAT:
    ...
    def get_tx_meter(self):
        """Return raw meter value while transmitting (0-255) or None."""
        if not self._send(b'\x00\x00\x00\x00\xe7'):
            return None
        resp = self._read(1)
        if not resp:
...
class StatusThread(QThread):
    status_updated = pyqtSignal(int, int, int, int)  # freq Hz, S-meter, mode, power meter
        ...
        while self.running:
            if self.cat.is_connected:
                ...
                if freq is not None and 100000 <= freq <= 500000000:
                    ...
                    self.status_updated.emit(freq, sm, mode, power)
            self.msleep(300)
...
class RadioControlApp(QMainWindow):
    ...
    def update_status(self, freq_hz, sm_level, mode, power_meter):
        if freq_hz is None:
            return
        ...
        self.update_power_label(band_label, power_meter)
        self.adjust_all_fonts()
    ...
    def update_power_label(self, band_label, meter_value):
        """Update power label in watts.

        While transmitting, the transceiver's meter value is scaled relative
        to the maximum power of the detected band. When not transmitting the
        label shows ``0 W`` so the displayed value reflects actual output.
        """

        # Determine nominal band power
        max_power = 0
        if band_label in {
            "160 m",
            "80 m",
        ...
        if self.cat.ptt_active and meter_value is not None and meter_value >= 0 and max_power > 0:
            # Scale the meter reading relative to the band's maximum power
            watts = round(max_power * meter_value / 255)
        else:
            watts = 0
