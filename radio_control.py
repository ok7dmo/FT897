...
class RadioControlApp(QMainWindow):
    ...
    def update_status(self, freq_hz, sm_level, mode):
        ...
        self.update_power_label(band_label)
        self.adjust_all_fonts()
    ...
    def update_power_label(self, band_label):
        power = "---"
        if band_label in {"160 m", "80 m", "60 m", "40 m", "30 m", "20 m", "17 m", "15 m", "12 m", "10 m", "6 m"}:
            power = 100
        elif band_label == "2 m":
            power = 50
        elif band_label == "70 cm":
            power = 20
        self.power_label.setText(f"Výkon: {power} W" if isinstance(power, int) else f"Výkon: {power}")
