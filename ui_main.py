...
class RadioControlApp(QMainWindow):
    ...
    def init_menu(self):
        ...
        vfo_act.triggered.connect(lambda: self.worker and self.worker.toggle_vfo())
        cmd_menu.addAction(vfo_act)

        split_on_act = QAction("Split ON", self)
        split_on_act.triggered.connect(lambda: self.worker and self.worker.split_on())
        cmd_menu.addAction(split_on_act)

        split_off_act = QAction("Split OFF", self)
        split_off_act.triggered.connect(lambda: self.worker and self.worker.split_off())
        cmd_menu.addAction(split_off_act)

        clar_on_act = QAction("Clarifier ON", self)
        clar_on_act.triggered.connect(lambda: self.worker and self.worker.clarifier_on())
        cmd_menu.addAction(clar_on_act)

        clar_off_act = QAction("Clarifier OFF", self)
        clar_off_act.triggered.connect(lambda: self.worker and self.worker.clarifier_off())
        cmd_menu.addAction(clar_off_act)

        rpt_plus_act = QAction("Repeater +", self)
        rpt_plus_act.triggered.connect(lambda: self.worker and self.worker.repeater_plus())
        cmd_menu.addAction(rpt_plus_act)

        rpt_minus_act = QAction("Repeater -", self)
        rpt_minus_act.triggered.connect(lambda: self.worker and self.worker.repeater_minus())
    ...
    def goto_band(self, freq_hz: int, mode_code: int):
        ...
        self.worker.set_mode(mode_code)
    ...
    def set_power_level(self, band_label: str, watts: int) -> None:
        ...
        if self.worker:
            self.worker.set_power(band_label, watts)
