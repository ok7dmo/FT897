...
# Base directory of the Hamlib installation on Windows.  The DLLs live in
# the "bin" subdirectory, so we add that to the DLL search path.
HAMLIB_ROOT = r"C:\\Program Files\\hamlib-w64-4.6.3"
if os.name == "nt":
    try:
        dll_dir = os.path.join(HAMLIB_ROOT, "bin")
...
required = ["pyserial", "PyQt5", "Hamlib"]
...
class FT897CAT:
    ...
    def connect(self, port, baudrate=9600):
        ...
        try:
            ...
            if self.hamlib:
                try:
                    self.rig = self.hamlib.Rig(self.hamlib.RIG_MODEL_FT897)
                    self.rig.set_conf("rig_pathname", port)
                    self.rig.set_conf("serial_speed", str(baudrate))
                    self.rig.open()
                except Exception as e:
                    print(f"Hamlib open failed: {e}")
                    self.rig = None
    ...
    def set_repeater_offset_mode(self, mode: str):
        ...
        if self.rig:
            ham_map = {
                "minus": self.hamlib.RIG_RPT_SHIFT_MINUS,
                "plus": self.hamlib.RIG_RPT_SHIFT_PLUS,
                "simplex": self.hamlib.RIG_RPT_SHIFT_NONE,
            }
            shift = ham_map.get(mode)
            if shift is not None:
                try:
                    self.rig.set_rptr_shift(shift)
                    return True
                except Exception as e:
                    print(f"Hamlib repeater shift error: {e}")
        mapping = {
            "minus": 0x09,
            "plus": 0x49,
            "simplex": 0x89,
    ...
    def set_repeater_offset_frequency(self, freq_hz: int):
        ...
        if self.rig:
            try:
                self.rig.set_rptr_offs(freq_hz)
                return True
            except Exception as e:
                print(f"Hamlib repeater offset error: {e}")
        units_10hz = int(freq_hz // 10)
        digits = f"{units_10hz:08d}"
        bcd = bytearray()
        for i in range(0, 8, 2):
