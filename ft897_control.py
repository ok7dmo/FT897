...
class FT897CAT:
    ...
    def get_firmware_version(self):
        """Request the radio firmware version (returns text or hex string)."""
        if not self.is_connected:
            return None
        if not self._send(b"\x00\x00\x00\x00\x19"):
            return None
        resp = self._read(4)
        if not resp:
            return None
    ...
    def power_on(self):
        if not self.is_connected:
            return False
        # Send the Hamlib-style wake sequence followed by the PWR ON command
        with self._lock:
            try:
                self.serial_port.write(b"\xff" * 5)
                self.serial_port.flush()
                time.sleep(0.2)
                self.serial_port.write(b"\x00\x00\x00\x00\x0f")
                self.serial_port.flush()
                return True
            except Exception as e:
                print(f"Chyba power on: {e}")
                return False

    def power_off(self):
        if not self.is_connected:
            return False
        return self._send(b"\x00\x00\x00\x00\x8f")
...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.band_configs = {
            ...
            "20 m": {"list": [14_070_000, 14_074_000, 14_200_000], "mode": "USB"},
            "17 m": {"range": (18_068_000, 18_168_000), "step": 1000, "mode": "USB"},
            "15 m": {"list": [21_070_000, 21_074_000, 21_200_000], "mode": "USB"},
    ...
    def init_menu(self):
        ...
        version_action = QAction("Zobrazit verzi rádia", self)
        version_action.triggered.connect(self.show_radio_version)
        cat_menu.addAction(version_action)
        ...
        power_on = QAction("Zapnout rádio", self)
        power_on.triggered.connect(self.power_on_radio)
        cat_menu.addAction(power_on)

        power_off = QAction("Vypnout rádio", self)
        power_off.triggered.connect(self.power_off_radio)
        cat_menu.addAction(power_off)
    ...
    def show_radio_version(self):
        version = self.cat.get_firmware_version()
        if version is None:
            QMessageBox.information(self, "Info", "Nepodařilo se zjistit verzi rádia.")
        else:
            QMessageBox.information(self, "Verze rádia", version)

    def power_on_radio(self):
        if not self.cat.power_on():
            QMessageBox.warning(self, "Chyba", "Nepodařilo se zapnout rádio.")

    def power_off_radio(self):
        if not self.cat.power_off():
            QMessageBox.warning(self, "Chyba", "Nepodařilo se vypnout rádio.")
