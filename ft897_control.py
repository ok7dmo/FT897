...
class FT897CAT:
    ...
    def get_power_level(self):
        """Return raw power setting (0-15) using TX status command."""
        if not self.is_connected:
            return None
        with self._lock:
            try:
                self.serial_port.reset_input_buffer()
                self.serial_port.reset_output_buffer()
                self.serial_port.write(b'\x00\x00\x00\x00\xf7')
                time.sleep(0.1)
                resp = self.serial_port.read(1)
            except Exception as e:
...
class RadioControlApp(QMainWindow):
    ...
    def update_status(self, freq_hz, sm_level, power_level):
        ...
        if power_level >= 0:
            ...
            watts = int(round(power_level / 15 * max_pwr))
