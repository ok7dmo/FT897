...
class FT897CAT:
    ...
    def power_on(self):
        if not self.is_connected:
            return False
        # Hamlib sends a wake sequence before the actual power-on command.
        with self._lock:
            try:
                self.serial_port.write(b"\xff\xff\xff\xff\xff")
                time.sleep(0.2)
...
class RadioControlApp(QMainWindow):
    ...
    def init_menu(self):
        ...
        for mode in ["LSB", "USB", "CW", "CWR", "AM", "FM", "DIG", "PKT"]:
        ...
        power_on.triggered.connect(self.cat.power_on)
        cat_menu.addAction(power_on)

        power_off = QAction("Vypnout rádio", self)
        power_off.triggered.connect(self.cat.power_off)
