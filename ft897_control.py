...
class FT897CAT:
    ...
    def set_power(self, watts: int):
        """Attempt to set transmitter power. The FT-897 does not
        officially support this via CAT, so this method currently
        prints a warning and returns False."""
        print("Setting power via CAT is not supported on the FT-897")
        return False
...
class RadioControlApp(QMainWindow):
    ...
    def init_menu(self):
        ...
        power_action = QAction("Nastavit výkon…", self)
        power_action.triggered.connect(self.show_power_dialog)
        settings_menu.addAction(power_action)
    ...
    def show_power_dialog(self):
        """Allow the user to set a desired power level for the current band.
        Actual power change is attempted via CAT but may not be supported."""
        if self.last_valid_frequency is None:
            QMessageBox.information(self, "Informace", "Nejprve získejte frekvenci z rádia.")
            return
        band = self.get_band_label_from_khz(self.last_valid_frequency / 1000)
        max_pwr = self.band_max_power.get(band)
        min_pwr = self.band_min_power.get(band, 5)
        if max_pwr is None:
            QMessageBox.information(self, "Informace", "Nastavení výkonu pro toto pásmo není k dispozici.")
            return

        dialog = QDialog(self)
        dialog.setWindowTitle(f"Nastavit výkon ({band})")
        layout = QVBoxLayout()
        spin = QSpinBox()
        spin.setRange(min_pwr, max_pwr)
        spin.setSuffix(" W")
        spin.setValue(max_pwr)
        ...
        if dialog.exec_() == QDialog.Accepted:
            watts = spin.value()
            if not self.cat.set_power(watts):
                QMessageBox.information(self, "Info", "Rádio nepodporuje nastavení výkonu přes CAT.")
