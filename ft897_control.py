...
class FT897CAT:
    ...
    def get_swr(self) -> bool:
        """Return True if the high VSWR bit is set in the RX status."""
        if not self.is_connected:
            return False
        with self._lock:
            try:
                ...
                print(f"Chyba při čtení SWR: {e}")
                return False

        if not resp:
            return False
        return bool(resp[0] & 0x40)
