...
def add_default_rigctl_path():
    """Ensure rigctl is in PATH on Windows using typical install dirs."""
    if shutil.which("rigctl"):
        return
    possible = [
        r"C:\\Program Files\\hamlib-w64-4.6.3\\bin",
...
add_default_rigctl_path()



class FT897CAT:
    """CAT control for the Yaesu FT-897 with rigctl, hamlib, or pyserial."""
    ...
    def connect(self, port, baudrate=9600):
        ...
        if os.name == "nt" and shutil.which("rigctl") is None:
            for d in [r"C:\\Program Files\\hamlib-w64-4.6.3\\bin", r"C:\\Program Files\\Hamlib\\bin"]:
                if os.path.isdir(d) and d not in os.environ.get("PATH", ""):
                    os.environ["PATH"] += os.pathsep + d
                    if shutil.which("rigctl"):
                        break
        ...
        if shutil.which("rigctl"):
            try:
                subprocess.check_call([
                    "rigctl", "-m", "1023", "-r", port, "-s", str(baudrate)
                ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                self.use_rigctl = True
                self.is_connected = True
                return True
            except Exception as e:
                print(f"rigctl connection failed: {e}")

        if hamlib is not None:
            try:
                self.rig = hamlib.Rig(hamlib.RIG_MODEL_FT897)
                ...
                print(f"hamlib connection failed: {e}")
        ...
        if self.use_rigctl:
            try:
                out = subprocess.check_output([
                    "rigctl", "-m", "1023", "-r", self.port, "-s", str(self.baudrate), "f"
    ...
    def ptt_on(self):
        ...
        if self.use_rigctl:
            try:
                subprocess.check_call([
                    "rigctl", "-m", "1023", "-r", self.port, "-s", str(self.baudrate), "T", "1"
                ], stderr=subprocess.DEVNULL)
                return True
            except Exception:
                return False
            ...
            try:
                return self.rig.set_ptt(hamlib.RIG_PTT_ON) == 0
            except Exception:
                return False
        else:
    ...
    def ptt_off(self):
        ...
        if self.use_rigctl:
            try:
                subprocess.check_call([
                    "rigctl", "-m", "1023", "-r", self.port, "-s", str(self.baudrate), "T", "0"
                ], stderr=subprocess.DEVNULL)
                return True
            except Exception:
                return False
            ...
            try:
                return self.rig.set_ptt(hamlib.RIG_PTT_OFF) == 0
            except Exception:
                return False
        else:
