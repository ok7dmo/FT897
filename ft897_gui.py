...
def add_default_rigctl_path():
    """Ensure the standard Hamlib path is in PATH on Windows."""
    if shutil.which("rigctl"):
        return
    if sys.platform.startswith("win"):
        default_dir = r"C:\\Program Files\\Hamlib\\bin"
        rigctl_exe = os.path.join(default_dir, "rigctl.exe")
        if os.path.exists(rigctl_exe) and default_dir not in os.environ.get("PATH", ""):
            os.environ["PATH"] += os.pathsep + default_dir
    ...
    try:
        if sys.platform.startswith("win"):
            if shutil.which("choco"):
                subprocess.check_call(["choco", "install", "-y", "hamlib"])
            elif shutil.which("winget"):
                subprocess.check_call(["winget", "install", "--id", "hamlib", "-e"])
            else:
                print("Warning: Install Hamlib (rigctl) manually from the official source.")
        ...
        print(f"Chyba instalace rigctl: {e}")
    if not shutil.which("rigctl"):
        print("Varování: CLI nástroj 'rigctl' nebyl nalezen ani po pokusu o instalaci.")
