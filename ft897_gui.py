...
class RadioControlApp(QMainWindow):
    ...
    def adjust_label_font(self, label: QLabel):
        if label == self.freq_label:
            text = "999,99999"
        elif label == self.smeter_label:
            text = "S-metr: S9+60"
        else:
            text = label.text()
        if not text:
        ...
        label.setFont(font)
        if label == self.smeter_label:
            self.cached_smeter_font = font
        elif label == self.freq_label:
            self.cached_freq_font = font
