...
def add_custom_rigctl_path():
    """Add the predefined Hamlib path to PATH if rigctl isn't found."""
    if shutil.which("rigctl"):
        return
    if sys.platform.startswith("win"):
        custom_dir = r"C:\\Program Files\\hamlib-w64-4.6.3\\bin"
        rigctl_exe = os.path.join(custom_dir, "rigctl.exe")
        if os.path.exists(rigctl_exe) and custom_dir not in os.environ.get("PATH", ""):
            os.environ["PATH"] += os.pathsep + custom_dir


add_custom_rigctl_path()
