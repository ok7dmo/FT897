...
class RadioControlApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        band_controls = QFrame()
        bc_layout = QVBoxLayout(band_controls)
        self.band_up_btn = QPushButton("Pásmo nahoru")
        self.band_up_btn.clicked.connect(lambda: self.change_band(1))
        self.band_down_btn = QPushButton("Pásmo dolů")
        self.band_down_btn.clicked.connect(lambda: self.change_band(-1))
        self.band_home_btn = QPushButton("Pásmo domů")
        self.band_home_btn.clicked.connect(self.band_home)
        bc_layout.addWidget(self.band_up_btn)
        bc_layout.addWidget(self.band_down_btn)
        bc_layout.addWidget(self.band_home_btn)
        self.layout.addWidget(band_controls)
    ...
    def change_band(self, direction):
        ...
        idx = None
        for i, ((start, end), _) in enumerate(self.band_definitions):
            if start <= freq_khz <= end:
                idx = i
                break
        if idx is None:
        ...
        if 0 <= new_idx < len(self.band_definitions):
            new_start = self.band_definitions[new_idx][0][0]
            self.cat.set_frequency(int(new_start * 1000))
