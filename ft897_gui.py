...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        # (label, frequency in Hz, mode code, TX power percent).
        freqs_80m = [
            (f"{freq/1000:.3f} MHz", freq * 1000, 0x03, 100)
        ...
        self.band_presets = {
            "80 m": freqs_80m,
            "40 m": freqs_40m,
            "20 m": [("14.250 MHz", 14250000, 0x01, 100)],
            "15 m": [("21.200 MHz", 21200000, 0x01, 100)],
            "10 m": [("28.400 MHz", 28400000, 0x01, 100)],
            "6 m": [("50.150 MHz", 50150000, 0x01, 100)],
            "2 m": [("145.500 MHz", 145500000, 0x08, 100)],
            "70 cm": [("433.500 MHz", 433500000, 0x08, 100)],
        }
        self.init_ui()
    ...
    def init_menu(self):
        ...
        for b_label, items in self.band_presets.items():
            ...
            for name, freq, mode, power in items:
                act = QAction(name, self)
                act.triggered.connect(lambda _=False, f=freq, m=mode, p=power: self.goto_band(f, m, p))
                sub.addAction(act)
    ...
    def goto_band(self, freq_hz, mode_code, power_percent):
        if not self.cat.is_connected:
            return
        self.cat.set_frequency(freq_hz)
        self.cat.set_mode(mode_code)
        self.cat.set_power(power_percent)
