...
class FT897CAT:
    ...
    def get_smeter(self):
        """Return (S-meter value 0-255, squelch_open flag) or (None, None)."""
        if self.ptt_active:
            return None, None
        with self._lock:
            try:
        ...
        if not resp:
            return None, None
        raw = resp[0]
        squelch_open = bool(raw & 0x02)
        s_raw = (raw & 0xF8) >> 3
        level = s_raw * 8
        return level, squelch_open
...
class StatusThread(QThread):
    status_updated = pyqtSignal(object, int, int)  # frequency, smeter, squelch
        ...
        while self.running:
            if self.cat.is_connected:
                ...
                if not self.cat.ptt_active:
                    sm, squelch = self.cat.get_smeter()

                if freq is not None or sm is not None or squelch is not None:
                    sm_val = sm if sm is not None else -1
                    sq_val = 1 if squelch else 0 if squelch is not None else -1
                    self.status_updated.emit(freq, sm_val, sq_val)
            self.msleep(300)
...
class RadioControlApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.tabs.addTab(self.squares_tab, "Squares")
    ...
    def update_status(self, freq_hz, sm_level, squelch_open):
