...
class RadioControlApp(QMainWindow):
    def __init__(self):
        ...
        self.prev_freq_font_size = None
        self.prev_band_font_size = None
    ...
    def _initial_setup(self):
        # Give the labels generous starting space so the fonts can be large
        self.freq_label.setMinimumSize(800, 200)
        self.smeter_label.setMinimumSize(800, 200)
        self.band_label.setMinimumHeight(100)

        self.adjust_all_fonts(fix_min=True)

        self.adjustSize()
        self.setMinimumSize(self.size())
        self.sync_tab_minimums()
    ...
    def init_ui(self):
        ...
        self.band_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def adjust_all_fonts(self, fix_min=False):
        ...
        def best_size(label, text):
            ...
            if width <= 0 or height <= 0:
                return 10
            # Search a wide range of point sizes so the labels can become very
            # large if the window permits
            low, high = 1, 500
            best = low
        ...
        # keep both main labels the same size and remember it for later
        common_size = min(freq_size, sm_size)
        if self.prev_freq_font_size != common_size:
            self.prev_freq_font_size = common_size
            ...
            freq_font = self.cached_freq_font

        if self.prev_band_font_size != band_size:
            self.prev_band_font_size = band_size
            band_font = QFont(self.current_font_family, band_size, QFont.Bold)
            self.band_label.setFont(band_font)
        else:
            band_font = self.band_label.font()

        if fix_min:
            metrics = QFontMetrics(freq_font)
            freq_rect = metrics.boundingRect(sample_freq)
            sm_rect = metrics.boundingRect(sample_sm)
            band_metrics = QFontMetrics(band_font)
            band_rect = band_metrics.boundingRect(sample_band)
            margin = 20
            min_w = max(freq_rect.width(), sm_rect.width()) + margin
            min_h = max(freq_rect.height(), sm_rect.height()) + margin
            self.freq_label.setMinimumWidth(min_w)
            self.smeter_label.setMinimumWidth(min_w)
            self.freq_label.setMinimumHeight(min_h)
            self.smeter_label.setMinimumHeight(min_h)
            self.band_label.setMinimumWidth(band_rect.width() + margin)
            self.band_label.setMinimumHeight(band_rect.height() + margin)

    def sync_tab_minimums(self):
        """Ensure all tab pages share the same minimum size."""
        size1 = self.freq_tab.sizeHint()
        size2 = self.smeter_tab.sizeHint()
        max_w = max(size1.width(), size2.width())
        max_h = max(size1.height(), size2.height())
        self.freq_tab.setMinimumSize(max_w, max_h)
        self.smeter_tab.setMinimumSize(max_w, max_h)
        self.tabs.setMinimumSize(max_w, max_h)
