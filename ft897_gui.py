...
required = ["pyserial", "PyQt5", "pyhamlib"]
...
class FT897CAT:
    """Minimal CAT control for the Yaesu FT-897 using Hamlib."""
    ...
    def connect(self, port, baudrate=9600):
        try:
            ...
            self.rig.set_conf(hamlib.conf.RIG_CONF_BAUDRATE, baudrate)
            self.rig.open()
    ...
    def ptt_on(self):
        try:
            return self.rig.set_ptt(hamlib.RIG_PTT_ON)
        except Exception:
            return False

    def ptt_off(self):
        try:
            return self.rig.set_ptt(hamlib.RIG_PTT_OFF)
        except Exception:
            return False
