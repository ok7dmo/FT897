...
class RadioControlApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # enlarge band label for better readability (50% bigger)
        self.band_label.setStyleSheet("font-size: 33px; font-weight: bold;")
        ...
        # make PTT button more visible (50% larger)
        self.ptt_btn.setFont(QFont("Arial", 30, QFont.Bold))
    ...
    def adjust_all_fonts(self):
        ...
        # determine longer sample text
        test_font = QFont(self.current_font_family, 10)
        fm = QFontMetrics(test_font)
        rect_freq = fm.boundingRect(sample_freq)
        rect_sm = fm.boundingRect(sample_sm)
        sample = sample_sm if rect_sm.width() * rect_sm.height() >= rect_freq.width() * rect_freq.height() else sample_freq
        ...
        def best_size(label):
            ...
            while low <= high:
                ...
                rect = metrics.boundingRect(sample)
        ...
        size = min(best_size(self.freq_label), best_size(self.smeter_label))
        # try to enlarge the font slightly if there is headroom
        # attempt to increase font by about 50% if space allows
        scaled = int(size * 1.5)
        if scaled > size:
            test_font = QFont(self.current_font_family, scaled)
            metrics = QFontMetrics(test_font)
            rect = metrics.boundingRect(sample)
            if (rect.width() <= self.freq_label.width() and
                    rect.height() <= self.freq_label.height() and
                    rect.width() <= self.smeter_label.width() and
                    rect.height() <= self.smeter_label.height()):
                size = scaled
        self.cached_main_font = QFont(self.current_font_family, size, QFont.Bold)
        self.freq_label.setFont(self.cached_main_font)
        self.smeter_label.setFont(self.cached_main_font)
