...
class RadioControlApp(QMainWindow):
    ...
    def adjust_all_fonts(self):
        ...
        common_font = QFont(self.current_font_family, freq_size, QFont.Bold)
        self.cached_freq_font = common_font
        self.cached_smeter_font = common_font

        self.freq_label.setFont(common_font)
        self.smeter_label.setFont(common_font)

        metrics = QFontMetrics(common_font)
        text = self.smeter_label.text()
        rect = metrics.boundingRect(text)
        margin_w = (
            self.central.contentsMargins().left()
            + self.central.contentsMargins().right()
            + 20
        )
        margin_h = (
            self.central.contentsMargins().top()
            + self.central.contentsMargins().bottom()
            + 40
        )
        new_width = max(rect.width(), self.freq_label.width()) + margin_w
        new_height = rect.height() + self.freq_label.height() + margin_h
